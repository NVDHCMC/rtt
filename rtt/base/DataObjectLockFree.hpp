...
{ namespace base {
    ...
    private:
        ...
        struct DataBuf {
            DataBuf()
                : data(), status(NoData), read_counter(), oro_write_lock(), next()
            {
                oro_atomic_set(&read_counter, 0);
                oro_atomic_set(&oro_write_lock, -1);
            }
            value_t data;
            FlowStatus status;
            mutable oro_atomic_t read_counter, oro_write_lock;
            DataBuf* next;
        };
